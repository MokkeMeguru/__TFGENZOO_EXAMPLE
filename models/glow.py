...
class Glow(Model):
    ...
    def inverse(
        self, x: tf.Tensor, zaux: tf.Tensor, training: bool, temparature: float = 1.0
    ):
        """inverse
        latent -> object
        """
        ...
        for flow in reversed(self.flows):
            if isinstance(flow, Squeeze):
                ...
                if flow.with_zaux:
                    ...
                    x = flow(x, zaux=zaux, inverse=True, temparature=temparature)
            else:
                x, ldj = flow(x, inverse=True, training=training)
                inverse_log_det_jacobian += ldj
        return x, inverse_log_det_jacobian

    def forward(self, x: tf.Tensor, training: bool, **kwargs):
        """forward
        object -> latent
        """
        zaux = None
